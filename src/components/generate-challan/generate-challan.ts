...
export class GenerateChallanComponent {
  ...
  getInfo() {
    this.showLoading();
    this.people.getVehicleDetails(this.vehicleNo).subscribe(
      (response: any) => {
        if (response !== 204) {
          ...
          const alert: Alert = this.alertCtrl.create({
            ...
            buttons: [
              ...
              },
              {
                text: "Yes",
                handler: () => {
                  const overlayView = this.appCtrl._appRoot._overlayPortal
                    ._views[0];
  ...
  viewViolations() {
    const violenterModal = this.modalCtrl.create(ViolenterHistoryPage, {
      data: this.violenter
    });
