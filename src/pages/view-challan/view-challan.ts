...
export class ViewChallanPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad ViewChallanPage');
  }

  getChallan(){
    const date:any = {};
    date.fromDate = this.date.fromDate.slice(0,-1).slice(0,10) + ' ' +  this.date.fromDate.slice(0,-1).slice(11);
    date.toDate = this.date.toDate.slice(0,-1).slice(0,10) + ' ' +  this.date.toDate.slice(0,-1).slice(11);
  ...
  getChallanDetail(challan) {
    ...
    this.navCtrl.push(PrintReceiptPage, {data : challan, currentViolents: currentViolents})
  }
